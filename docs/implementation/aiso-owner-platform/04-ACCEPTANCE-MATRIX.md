...
| **AC-14** | Mobile review, approve and request-changes | **BLOCKED (on one human action, and only that)** | Unchanged in substance, precise in shape. CI runs Playwright including the Pixel-5 project, but under `E2E_FIXTURE_MODE` against a fixture DSN where `getProfile()` returns null — and every existing spec renders pre-built HTML into a blank page while aborting all network. That is a real check of markup at a viewport; it is not a journey. Nothing behind `requireAuth` is reached, no route handler runs, no query executes. **Everything that does not depend on the human step is now built.** `tests/e2e/authenticated/owner-review.spec.ts` walks the live app on a Pixel 5: Home’s priorities section, the approved-facts surface (asserting the gate sentence and a 40px minimum target on every control), and a work-item version’s decision controls. **The last of those three asserted nothing until 2026-09-11:** `VersionWorkspace` seeds `selected` from an `initialVersion` prop the page never passes and has no effect that auto-selects, so the decision block is absent on first paint and `getByRole('button').count() > 0` was satisfied by Submit/Reload-draft/Reload-history alone. It now requires a submitted version to exist, clicks it, proves the detail region opened, and then **branches**: the approve/request-changes options plus the reason field, each measured against 40px, when `can_decide` holds — or the separation-of-duties denial sentence when it does not. Branching is not hedging: `can_decide` requires the actor to be neither the submitter nor a non-approver (AC-08), so a lone owner reviewing their own version correctly gets the denial, and asserting the controls unconditionally would fail against a correct product. The touch-target loop on the sources test covers `getByRole('button')` only, which excludes a combobox and a textbox — the two controls AC-14 is named after were the ones never measured. It runs under a dedicated `authenticated-mobile` Playwright project that exists **only** when a captured session file does — verified in both directions: with one, seven projects and three authenticated tests; without, the project list is byte-identical to before and `chromium`/`mobile` stay at 293 each, so CI is untouched and no spec leaked. `tests/fixtures/auth.ts` no longer throws unconditionally: it replays the captured session, proves it by reaching `/en/dashboard` before yielding, and still refuses rather than handing back an anonymous page. `npm run e2e:auth:capture` opens a browser and waits for a human to sign in — **it types nothing on anyone’s behalf**, and refuses before opening a browser if `NEON_AUTH_BASE_URL` is missing, since anything captured then would be an anonymous state file wearing an authenticated name. `npm run e2e:authenticated` preflights and refuses with every blocker named at once, rather than Playwright’s generic `project not found`. The check validates the value’s **shape**, not merely its presence, and both entry points share one definition: a Postgres DSN pasted into `NEON_AUTH_BASE_URL` parses as a URL and previously passed both scripts, which would have produced a browser, a silent sign-in failure and a ten-minute timeout naming nothing. It is now diagnosed by name, including that a DSN carries a password and belongs in `DATABASE_URL`. It also names the worktree trap, since `.env.local` is gitignored and setting the value in another checkout looks identical to not setting it. The session file is gitignored; it is a live credential. `__tests__/config/authenticated-e2e.test.ts` (**22 tests**, not the 11 this row claimed until 2026-09-11) pins most of it. **Three defects in that pinning were found and fixed on 2026-09-11, none of which needed the human step** — which falsifies the earlier claim on this row that everything not depending on it was built. (1) Both npm entry points ran as `node --env-file=.env.local`, and Node 24 exits 9 with `node: .env.local: not found` **before the module graph loads**, so `describeAuthConfig` never ran. `.env.local` is gitignored, so every fresh clone and every new worktree lands there — making the worktree blocker the code goes out of its way to write unreachable in the exact situation it is written for. `--env-file-if-exists` fixes it and is pinned in both directions. (2) "appears only when the state file exists ... verified in both directions" was pinned by two `toContain` greps over `playwright.config.ts` that target the line **defining** the gate, not the line using it: deleting the gate outright left all of them green. (3) Nothing anywhere referenced the authenticated spec, so deleting `owner-review.spec.ts` left every guard passing — green assertions around an empty directory, the precise "rots into decoration" outcome. Both are now behavioural, in `__tests__/config/playwright-projects.test.ts`, which asks Playwright rather than reading text; a stub `{"cookies":[],"origins":[]}` state file flips the gate because `storageState` is read at context creation, never at `--list`, so **discovery needs no session and only execution stays manual**. Proved by mutation, not by passing: deleting the gate fails exactly the first, moving the spec away fails exactly the second, and the 22 source-grep tests stay green under both — which is what made them worth replacing. **What remains is irreducibly human:** put `NEON_AUTH_BASE_URL` in `.env.local` (empty today), then sign in once. This product has **no password sign-in** — only magic link and Google OAuth — so no credential exists for a runner to present, and no configuration changes that. The specs **fail rather than skip** when the data they need is absent, including naming what to create when no work item exists to review — because a green run over an account with nothing to approve does not satisfy AC-14. |
...
| PARTIAL | 5 | AC-01, AC-03, AC-11, AC-12, AC-15 |
| BLOCKED | 1 | AC-14 |
| DEFERRED | 1 | AC-06 |
| FAIL | 0 | — |
