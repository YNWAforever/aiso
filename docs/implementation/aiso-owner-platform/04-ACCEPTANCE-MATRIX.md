...
| **AC-11** | Technical, search/AI and business outcomes stay separate | **PARTIAL** | `lib/outcomes` is clean: a technical verdict cannot populate a business one, because no business concept exists in that module — and none was added here, deliberately. The layer-mixing this row named lived in `lib/localTrust/roi.ts`, and it was worse than "a computed baseline shown to owners". **The figure cannot move with the score it was displayed under.** No caller passes `previousScore`, so the fallback baseline is always `score - 5`, the delta is always capped at 5, and the enquiry range is `[1, 2]` for *every* non-zero score from 1 to 100 — a client scoring 3 and a client scoring 97 saw the identical amount. It varies only with the average lead value and close rate the owner typed in themselves. The arithmetic was never wrong; the framing was. A panel titled **"ROI Proof Timeline"**, rendered directly beneath "Local Trust Score: 71/100", asserts that the score produced the money, and the code never reads a score movement at all. **Fixed this session as a labelling and disclosure change, with zero behaviour change:** proof language retired from seven keys in **both** catalogues (`roi_timeline` is now "Enquiry value scenario" / 「查詢價值情境」, `local_trust_timeline_estimate` is the taxonomy word "Estimated" / 「估算」); two new keys state the basis and the limitation, rendered in the **same block** as the number so it cannot be screenshotted apart from what qualifies it; the enquiry range is printed rather than hidden, which makes the `1-2` constant visible to the reader instead of implied by a heading; and the assumptions are coerced and formatted, because `average_lead_value` and `close_rate` are `numeric` columns that reach the view as strings. **The export was the riskiest surface and is fixed too:** the CSV emitted a bare `Estimated Value Low,1600` with no currency and no basis — a spreadsheet row that travels away from the panel and reads as a measured result. It now carries the currency, both assumption inputs, the enquiry range and a literal `Basis` row. All three existing export tests passed `roi_estimate: null`, so the money path had **no coverage at all**; it does now. `__tests__/lib/roi-scenario.test.ts` (16 tests) pins the constant range as the behaviour it actually is, so wiring a real baseline later becomes a visible change rather than a silent one. **Still PARTIAL, and deliberately:** the fabricated baseline itself is not removed. Deleting the `score - 5` fallback on its own would make `estimateRoi` return null in every production case, and the panel would show "Add average lead value and close rate..." to an owner who already did — the feature would be silently removed while appearing merely unconfigured. Supplying a real previous snapshot is its own change, with a real consequence to accept: every client's first month would then correctly show no estimate. |
