...
|---|---|---|---|
| **AC-01** | New owner gets a real baseline and understands three priorities | **PARTIAL** | Scan → result → claim is real and unit-tested. Home now leads with **at most three priorities and one named next action** (`lib/view-models/owner-priorities.ts`, rendered by `components/dashboard/WorkspaceHome.tsx`), ranked by points still at stake and derived from the evidence envelope so an unobservable check never becomes work to do; 30 tests across `__tests__/lib/owner-priorities.test.ts` and `__tests__/components/workspace-home-priorities.test.tsx`. **The first-run journey is now reachable, and Slice 10's telemetry has its first reader.** The portfolio's empty state was a dead-end sentence (`emptyBrands`); it now names the first step and links to `/{lang}/onboarding`, carrying the scan id recovered from the signed claim-intent cookie so onboarding can claim the scan the visitor just ran. That is the **first inbound link to that route anywhere in the product**, which matters more than it sounds: `/api/onboarding/complete` is the only path that claims the anonymous scan, starts the trial and seeds the question bank, and it could not be reached by using the product. `AddBrandWizard` is not the same path and never was — it posts to `/api/dashboard/clients`, which adds a brand and does none of those three. The cookie is **read, never consumed**; single-use consumption belongs to the claim itself, and spending it to render a link would leave the link unable to claim. Separately, `readActivation` had had **no caller since it was written**, so the derived funnel was computed for nobody: `lib/view-models/activation-progress.ts` projects it and `components/dashboard/ActivationProgress.tsx` renders it on the dashboard root in both languages, preserving both properties the derivation exists for — a failed read renders as `unavailable` rather than a zero (the distinction `readActivation` throws to protect), and a milestone reached out of order is **named rather than counted**, because counting it would report a funnel nobody walked and hiding it would make the panel contradict the rows. 8 projection tests, 14 render tests, 4 page tests. **Still partial:** none of this has been exercised against a database, and the journey it opens still cannot run in CI, where E2E runs under `E2E_FIXTURE_MODE` with `getProfile()` returning `null`. |
...
| **AC-06** | A website finding and a question opportunity reach the same asset and task | **PARTIAL** (asset and task built; unapplied) | Phase 3 per the plan, and the status is right — but **the reason this row gave until 2026-09-11 was false, and the row was its own only evidence.** It claimed `lib/opportunities/fingerprint.ts` "already dedupes by canonical target, so the seam exists". Three things are wrong with that. `opportunityKey` (`fingerprint.ts:200-202`) returns `${ruleVersion}:${source.kind}:${source.id}:${source.checkKey ?? ''}` — keyed by **source**, never by target; `canonicalize` in that file is deterministic JSON serialisation (key ordering, `-0`, cycles), a different sense of the word; and nothing in the file dedupes at all — `fingerprintEvidence` hashes, and every consumer (`work-items/service.ts`, `change-sets/*`, `delivery/export.ts`) uses the result as a staleness or integrity witness. A repo-wide grep for `canonical target|assetKey|targetKey|asset_key|target_key` returned exactly one hit: this row. **The real blockers, both structural.** *Asset:* there is no page-level target on either side to join on. A scan finding keeps `url.origin` and reduces path, query and fragment to booleans under `URL_REDACTION_VERSION = 'origin-only.v1'` (`lib/scan-evidence.ts`), so it names a **site, never a page**; a Pulse observation (`lib/observations/types.ts`) carries no url, page or citation field at all, and the pulse rule's args are exactly `{question, platform}`. There is also no "asset" in this schema — `grep -rni asset supabase/migrations/*.sql` returns nothing, and `client_entities` (`040`) is a brand `display_name` + `aliases` with no URL. *Task:* `evidence_work_items` is single-source by constraint — `evidence_work_items_rule_source_check` (`041:36-40`) binds `source_kind` to `rule_version` on the one row, and the only uniqueness is `unique (account_id, client_id, opportunity_key)` (`041:46`) over a source-derived key. **Do not "fix" this by making the two keys collide:** `saveAuthenticatedDraft` (`lib/work-items/service.ts`) returns the existing draft by that key *before* it loads or validates the second source, so the second finding's evidence snapshot is never built and the row would describe one source while claiming to represent two — the provenance lie migrations `041`–`046` exist to prevent. `__tests__/opportunities/ac-06-convergence.test.ts` (5 tests) pins the disjoint keyspaces, the single-source CHECK and the absence of any asset column, so this row cannot drift back into claiming a seam that is not there. **One asset signal the product already observes and nothing reads:** `ai_citation_log` (`012:51-63`) records `cited_url` and `cited_domain` for every URL a Pulse answer cited. It is write-only today — `app/api/pulse/run/route.ts` inserts, and no module in `lib/` or `app/` selects from it — and it cannot be joined to a metric anyway: it carries no `prompt_id`, no `scan_week`, and its `platform` CHECK uses a different vocabulary from `pulse_metrics.platform`. That is the nearest real starting point, and it is a Phase 3 change: a migration plus a decision nobody has made about which page answers a question. **What AC-06 is waiting on is that decision, not wiring.** **The decision was made on 2026-09-12 — registered pages — and the asset half is now built.** Migration `050` adds `client_assets` (an owner-registered page, with its `origin` stored alongside because that is the only column a scan finding can be matched against) and `client_asset_questions` (`this page answers this question`, declared by a person). A scan finding attaches by origin; a question attaches by declaration; `lib/view-models/asset-convergence.ts` brings both to the same page and `converges` is true only when both are present. **The scope label is the load-bearing part.** `origin-only.v1` kept no path, so a finding listed under a page is carried and rendered as `scope: 'site'`, with a sentence under the heading in both languages saying it was observed for the whole site and not for that page alone — the layout would otherwise become a claim the product never collected. One site finding therefore attaches to *every* registered page on that origin, which is correct rather than duplication: dropping it from the second page would under-report the site. **The task half is deliberately untouched.** `evidence_work_items` gains no asset column, and `050` mentions neither it nor `opportunity_key` — asserted, not merely intended, by five new cases in `__tests__/opportunities/ac-06-convergence.test.ts`. The trap that file documents is now *more* live, not less: with assets existing, "and merge the tasks too" looks like the obvious next step, and it is still the provenance lie `041`–`046` exist to prevent. **One tenancy limitation, stated where a reader will look:** `prompt_bank` predates tenancy — no `account_id`, and a plain FK to `clients(id)` — so the declaration's FK to it cannot be composite and the database cannot prove a prompt belongs to the account. `lib/assets/store.ts` proves it in SQL instead, joining the prompt on the asset's own `client_id` inside the insert; that join is the whole guarantee for the Pulse side, and the migration header says so. 31 unit tests on the pure layers, 15 route tests, 11 render tests in both languages at touch size. **The task half was built on 2026-09-13.** Migration `051` moves the seven source columns out of `evidence_work_items` into a new `work_item_sources` child table, one row per source, backfilled one-for-one from the existing single-source items; the moved CHECK is proven against `051`'s own text, not merely carried over by assumption, in the rewritten `__tests__/opportunities/ac-06-convergence.test.ts`. `lib/work-items/sources.ts`'s `attachSource`/`withdrawSource` put tenancy inside the statement itself (a chained-CTE insert-then-bump for attach, matching `createDraftIfEvidenceCurrent`'s own shape) and are reachable over HTTP at `app/api/dashboard/clients/[clientId]/work-items/[itemId]/sources/route.ts`, gated by `authorizeAssetRegistry` plus the `WORK_ITEM_MULTI_SOURCE_V1` flag (off means every item still has exactly one source, byte-identical to before `051`). Attaching re-derives the second source's evidence exactly as `saveAuthenticatedDraft` re-derives the first — `loadOwnedDraftSource` then `deriveSuggestions(...).find(...)` — and refuses with 409 when it no longer matches, closing the exact provenance-lie shortcut this row already named. Submission compares an aggregate of every live source's snapshot rather than one column (`lib/change-sets/store.ts`, `ReviewContent` now a `schemaVersion` 1/2 discriminated union so ~10 existing single-variant callers needed zero changes); a decision independently re-derives every live source's fingerprint a second time and refuses with the stale opportunity keys named, not just "cannot approve" (`lib/change-sets/service.ts`). `lib/assets/merge-suggestions.ts` pairs a scan finding and a declared question that land on the same registered page and surfaces it on the assets screen, saying plainly when the two already have separate items and cannot be merged — absorbing an existing drafted item into another is out of scope by design, not an oversight. **Three limits recorded, not closed:** absorbing two already-drafted items into one is explicitly out of scope (a terminal state for the absorbed item does not exist); "a work item keeps at least one live source" is enforced inside `withdrawSource`'s own statement rather than a CHECK constraint, so it depends on every writer going through that function; and a narrow window remains between a decision's fingerprint re-derivation and its write, exactly as it already did for the single-source case. **Still not done:** `051` is authored and has run only against disposable integration-test branches, never a persistent database — `npm run migrate -- --verify` has not been asked about it — so the row stays PARTIAL rather than PASS despite both halves now existing. |
...
| **AC-12** | No cross-account read, mutation, inference or export | **PARTIAL** | Proven against **real Postgres**, not SQL text: the five owner-loop suites exercise the composite-FK tenancy chain, the append-only GRANT posture, and the app role's inability to UPDATE or DELETE version, decision and delivery history. **Changed this session:** they now run **in the release gate**. `scripts/ci/run-exact-target-suites.mjs` provisions one disposable branch through the single audited path, derives every `C9*` value from that branch and from nothing else, runs all five configs, and deletes the branch in a `finally`. The existing `integration` job invokes it inside its own step, before the summary is written — a later step would have let `write-job-summary` record success before the wrapper had run. Measured, not estimated: **5/5 suites, 117 tests, exit 0, 105s** including branch create, all 46 migrations and teardown, against that job's 30-minute budget. Eleven assertions in `__tests__/ci/exact-target-suites.test.ts` stop it drifting back out (§5), and `scripts/ci/count-vitest-reports.mjs` replaces the hardcoded `--executed 1 --skipped 0` the summary used to publish. `__tests__/api/route-gate-inventory.test.ts` separately fails when a handler ships with no gate. **The older feature stores now have their negative proved, by construction.** `__tests__/security/tenancy-inventory.test.ts` inventories every SQL statement in `lib/` and `app/` — 188 of them — and requires each one touching a tenant-bearing table either to constrain the account in its own text or to sit in a function that proved ownership first. 17 remain unscoped and all 17 are declared, function by function, with the reason: ten keys, every one either cron-authenticated (the three `agents/*` callbacks, `markCompleteIfAllPresent`, alert evaluation, the Pulse scheduler and rollup) or public by design (the result page, its OG card, the onboarding pre-fill). Pulse, the prompt bank, Local Trust, the agent tables and authority overrides hold no unscoped cross-account read outside those. The naive rule — every statement must contain `account_id` — would have been mostly false positives, because the correct shape proves ownership against `clients` or `scans` one statement earlier and then addresses the row by the id that check just validated; so scope is the enclosing function, anchored to top-level declarations. Writing it surfaced two things worth keeping: the onboarding pre-fill **looked** like a cross-account read and is not — `buildPublicResultSummary` already returns `domain`, `industry` and `region` to anonymous viewers of the same id, so narrowing it would break the anonymous scan → sign-up funnel and close nothing — and the inventory's own first version was blind to `` db()`…` ``, the second tagged form, which carries 19 statements including every `notifications` query and the public result page's `select * from scans`; a matcher knowing one form reports those files as containing no SQL at all. Proved by mutation rather than by passing: four deliberate breakages, each run separately, all four caught (removing a live `account_id` predicate, deleting a declaration, dropping the second tag form, and adding an unscoped statement inside an already-declared function — the last caught only by the pinned total, which is why that assertion exists). **Still partial:** this proves the query text, not the behaviour. The five `C9*` suites remain the only cross-account proofs executed against real Postgres, and they still cover only the owner-loop tables. |
