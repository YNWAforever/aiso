...
interface PricingPageProps {
  /** Which checkout plans currently have a configured Stripe price. Computed
   *  server-side from lib/stripe.ts — only the boolean crosses the boundary,
   *  never a price id. A missing entry is treated as unpurchasable. */
  purchasable: Partial<Record<CheckoutPlanId, boolean>>
}
...
/* ── Main component ─────────────────────────────────────────── */
export function PricingPage({ purchasable }: PricingPageProps) {
  ...
  const startCheckout = async (planName: CheckoutPlanId) => {
    if (!purchasable[planName]) return
    setLoading(true)
    setCheckoutError('')
    try {
  ...
  const plans = CHECKOUT_PLAN_IDS.map(key => {
    ...
    const isPurchasable = purchasable[key] === true
    return {
      key,
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    disabled={loading || !plan.purchasable}
                  ...
                  >
                    {plan.cta}
                    {!loading && plan.purchasable && <ChevronRight aria-hidden="true" className="size-3.5" />}
                  </button>
                  {!plan.purchasable && (
                    <p className="mt-2 text-center text-xs text-muted-foreground">{t('plan_unavailable_note')}</p>
                  )}
                </div>
              )
            })}
