...
describe.skipIf(!optedIn)('change-set schema on an exact disposable target', () => {
  ...
  it('preserves frozen actor identities after profiles are deleted and restricts history parents', async () => {
    ...
    await expect(sql`delete from evidence_work_items where account_id=${account} and id=${workItemId}`).rejects.toMatchObject({code:'23503'})
    await expect(sql`delete from clients where account_id=${account} and id=${client}`).rejects.toMatchObject({code:'23503'})
    await expect(sql`delete from accounts where id=${account}`).rejects.toMatchObject({code:'23503'})
  })
})
