...
function rawPatchRequest(raw: string) {
  return new NextRequest('http://localhost/api/admin/clients', {
    method: 'PATCH',
    body: raw,
    headers: { 'Content-Type': 'application/json' },
...
describe('PATCH /api/admin/clients', () => {
  beforeEach(() => {
    ...
    vi.resetModules()
    getProfileMock.mockReturnValue(ADMIN)
    sqlMock.mockResolvedValue([{ id: VALID_ACCOUNT_ID }])
  })

  it('returns 403 for a non-admin', async () => {
    getProfileMock.mockReturnValue({ ...ADMIN, is_admin: false })
    expect((await patch({ accountId: VALID_ACCOUNT_ID, action: 'revoke' })).status).toBe(403)
  })

  it('grants an override and never writes accounts.plan', async () => {
    const res = await patch({
      accountId: VALID_ACCOUNT_ID, action: 'grant', plan: 'enterprise', reason: 'partner deal',
    })
    expect(res.status).toBe(200)
  ...
  it('takes override_set_by from the session, not the body', async () => {
    await patch({
      accountId: VALID_ACCOUNT_ID, action: 'grant', plan: 'pro', reason: 'support',
  ...
  it('accepts free as a downgrade comp', async () => {
    const res = await patch({
      accountId: VALID_ACCOUNT_ID, action: 'grant', plan: 'free', reason: 'abuse',
    })
    expect(res.status).toBe(200)
  })
  ...
  it('rejects a plan outside PLAN_IDS', async () => {
    const res = await patch({
      accountId: VALID_ACCOUNT_ID, action: 'grant', plan: 'platinum', reason: 'x',
    })
  ...
  it('rejects a grant with no reason', async () => {
    const res = await patch({
      accountId: VALID_ACCOUNT_ID, action: 'grant', plan: 'pro', reason: '  ',
    })
    expect(res.status).toBe(400)
  })
  ...
  it('rejects a reason over 500 characters', async () => {
    const res = await patch({
      accountId: VALID_ACCOUNT_ID, action: 'grant', plan: 'pro', reason: 'x'.repeat(501),
    })
    expect(res.status).toBe(400)
  })
  ...
  it('rejects an expiry in the past', async () => {
    const res = await patch({
      accountId: VALID_ACCOUNT_ID, action: 'grant', plan: 'pro', reason: 'x',
      expiresAt: '2020-01-01T00:00:00Z',
    })
    expect(res.status).toBe(400)
  })

  it('grants with a future expiresAt, storing a normalised ISO string', async () => {
    const res = await patch({
      accountId: VALID_ACCOUNT_ID, action: 'grant', plan: 'pro', reason: 'trial extension',
      expiresAt: '2099-01-01T00:00:00Z',
    })
    expect(res.status).toBe(200)
    const params = sqlMock.mock.calls[0].slice(1)
    expect(params).toContain(new Date('2099-01-01T00:00:00Z').toISOString())
  })

  it('revokes by nulling all four override columns', async () => {
    const res = await patch({ accountId: VALID_ACCOUNT_ID, action: 'revoke' })
    expect(res.status).toBe(200)
    const text = queryText(sqlMock.mock.calls[0][0]).toLowerCase()
  ...
  it('returns 404 when the account does not exist', async () => {
    sqlMock.mockResolvedValue([])
    const res = await patch({
      accountId: '00000000-0000-4000-8000-000000000000', action: 'revoke',
    })
  ...
  it('rejects a malformed accountId without reaching the database', async () => {
    const res = await patch({ accountId: 'not-a-uuid', action: 'revoke' })
    expect(res.status).toBe(400)
    expect(sqlMock).not.toHaveBeenCalled()
  })

  it('rejects a non-string accountId', async () => {
    const res = await patch({ accountId: 12345, action: 'revoke' })
    expect(res.status).toBe(400)
  })

  it('rejects a request with no action', async () => {
    const res = await patch({ accountId: VALID_ACCOUNT_ID })
    expect(res.status).toBe(400)
  })

  it('rejects a malformed JSON body', async () => {
    const { PATCH } = await import('@/app/api/admin/clients/route')
    const res = await PATCH(rawPatchRequest('{not valid json'))
    expect(res.status).toBe(400)
  })
})
