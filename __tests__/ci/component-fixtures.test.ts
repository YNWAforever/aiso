 ...
 it('generates every required slice with final CSS and renderer environment',async()=>{
  ...
  try{
   ...
   await prepareComponentFixtures(root,{NODE_ENV: 'test'},(_bin:string,args:string[],options:{env:NodeJS.ProcessEnv})=>{
    expect(args.filter(a=>a.endsWith('.test.tsx'))).toEqual(['workspace-home','portfolio','pulse-view','settings','entity','observation','opportunity','draft','change-set','approver-access'].map(name=>`__tests__/components/${name}-render.test.tsx`))
    for(const slice of ['C8A','C8B','C8C','C8G','C9A','C9B','C9C','C9C_DRAFT','C9D','C9D_APPROVERS']){expect(options.env[`${slice}_HTML_DIR`]).toBe(join(root,'.next/component-fixtures',slice));expect(options.env[`${slice}_CSS_PATH`]).toBe(join(root,'.next/component-fixtures/build.css'))}
    return {status:0}
   })
