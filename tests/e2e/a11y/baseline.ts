 ...
 *
 * The original baseline was recorded in CI. Counts can differ by rendering
 * platform, so never raise a local allowance to hide a failing scan. Inspect
 * the reported elements and fix the defect before lowering resolved counts.
 * On 2026-09-05, local regression tests reproduced white text on cyan pricing
 * controls; semantic foreground tokens resolved all dark pricing violations.
 * That supersedes the historical Windows/Linux count discrepancy for those
 * cells. Fresh CI verification remains separate from this local evidence.
 */
